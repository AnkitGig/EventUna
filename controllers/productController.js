  ...
  try {
    ...
    if (error)
      return res
        .status(400)
        .json({ status: false, message: error.details[0].message });
    const { name, description, price, categoryId } = req.body; // subcategoryId removed
    const category = await Category.findById(categoryId);
...
exports.updateProduct = async (req, res) => {
  try {
    const imageIds = parseJsonArray("imageIds", req);
    req.body.imageIds = imageIds;
    const { productId, name, description, price, categoryId } = req.body; // subcategoryId removed
